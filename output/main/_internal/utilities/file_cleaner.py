...
class FileCleaner:
    def __init__(self, master, verbose=False):
        ...
        self.freed_space_label = ctk.CTkLabel(main_frame, text="", font=("Roboto", 12, "italic"), text_color="#8F8F8F")
        self.freed_space_label.pack(pady=(0, 20))
        ...
        listbox_frame = ctk.CTkFrame(main_frame, fg_color="#3b3b3b", corner_radius=8)
        listbox_frame.pack(pady=15, padx=15, fill="both", expand=True)

        self.file_list_scrollbar = ctk.CTkScrollbar(listbox_frame)
        self.file_list = Listbox(listbox_frame, yscrollcommand=self.file_list_scrollbar.set, selectmode=MULTIPLE, height=10,
                                 font=("Roboto", 12), bg="#3b3b3b", fg="#ffffff", borderwidth=0, highlightthickness=0)
        self.file_list_scrollbar.configure(command=self.file_list.yview)
    ...
    def scan_files(self):
        ...
        self.freed_space_label.configure(text=f"Освободится: {total_size_mb:.2f} MB")
